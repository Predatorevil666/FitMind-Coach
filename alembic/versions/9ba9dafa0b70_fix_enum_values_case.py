"""fix_enum_values_case

Revision ID: 9ba9dafa0b70
Revises: 70202eb
Create Date: 2025-09-01 19:50:33.123456
...
revision: str = '9ba9dafa0b70'
down_revision: Union[str, None] = '70202eb'
...
def upgrade() -> None:
    ...
    op.execute("""
        ...
        END
        WHERE goal IN (
            'WEIGHT_LOSS', 'MUSCLE_GAIN', 'ENDURANCE', 'HEALTH', 'OTHER'
        ) OR goal != LOWER(goal);
    """)
    ...
    op.execute("""
        ...
        ) OR workout_type != LOWER(workout_type);
    """)
    ...
    op.execute("""
        ...
        END
        WHERE meal_type IN ('BREAKFAST', 'LUNCH', 'DINNER', 'SNACK', 'OTHER')
           OR meal_type != LOWER(meal_type);
    """)
...
def downgrade() -> None:
    ...
    op.execute("""
        ...
        SET goal = CASE 
            ...
            ELSE UPPER(goal)
        END
        WHERE goal IN ('weight_loss', 'muscle_gain', 'endurance', 'health', 'other')
           OR goal != UPPER(goal);
    """)
    ...
    op.execute("""
        ...
        END
        WHERE workout_type IN ('strength', 'cardio', 'flexibility', 'hiit', 'other')
           OR workout_type != UPPER(workout_type);
    """)
